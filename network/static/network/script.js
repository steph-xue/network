...
function edit(id) {
    ...
    .then(result => {
        ...
        // Directly modifies the content of the post to reflect the edits, without needing to refresh
...
function like_handler(id) {
    ...
    // Using an API, checks the user's like status of the post
    fetch(`/like_status/${id}`)
    .then(response => response.json())
    .then(result => {

        // Gets the user's like status of the post
        let liked = result["liked"];

        // If the post has not yet been liked, like the post
        if (!liked) {

            // Using an API, changes the post to add a like and save it to the database
            fetch(`/add_like/${id}`)
            .then(response => response.json())
            .then(result => {
                ...
                // Using an API, determines the number of likes the post has to update the post's likes directly
                fetch(`/like_count/${id}`)
                .then(response => response.json())
                .then(result => {
        ...
        // If the post has already been liked, remove the like from the post
            ...
            // Using an API, changes the post to remove the like and save it to the database
...
function dislike_handler(id) {
    ...
    // Using an API, checks the user's dislike status of the post
    fetch(`/dislike_status/${id}`)
    .then(response => response.json())
    .then(result => {

        // Gets the user's dislike status of the post
        let disliked = result["disliked"];

        // If the post has not yet been disliked, dislike the post
        if (!disliked) {

            // Using an API, changes the post to add a dislike and save it to the database
            ...
            .then(result => {
                ...
                // Using an API, determines the number of dislikes the post has to update the post's dislikes directly
        ...
        // If the post has already been disliked, remove the dislike from the post
            ...
            // Using an API, changes the post to remove the dislike and save it to the database
